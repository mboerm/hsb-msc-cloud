...
public class StorageServiceCreator implements IServiceCreator {
    ...
    public StorageServiceCreator(String name, String type, String mode, int capacity, int data, Pair<Integer, Integer> requests, int rate) {
        this.name = name;
        this.type = type;
        this.mode = mode;
    ...
    public Service createService() {
        ...
        if (type.equals(types[0])) {
            storageService = new StorageService(name, type, mode, capacity, data, requests, 0);
        } else if (type.equals(types[1])) {
            storageService = new StorageService(name, type, mode, capacity, data, new Pair<>(0,0), 0);
        } else if (type.equals(types[2])) {
            storageService = new StorageService(name, type, mode, capacity, data, new Pair<>(0,0), rate);
        } else {
            return null;
        }
